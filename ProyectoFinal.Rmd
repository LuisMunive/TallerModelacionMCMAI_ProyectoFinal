...
MHdentroGibbs <- function(theta, N){
  ...
  for(i in 2:N){
    ...
    mu2[i] <- rnorm(1, mean = lambda2_n^(-1) * (lambda2_0 * mu2[i - 1] + n * mean(r[i - 1, ] * sin(theta))), sd = 1 / sqrt(lambda2_n))
     ...
     r[i,] <- MetroHast(f = kerY, theta = theta, mu = c(mu1[i], mu2[i]), burn = 2500, ite.inter = 500, lag = 1, tmues = 1, n)
  }
...
tiempoInicio <- Sys.time()
  ...
  prueba <- MHdentroGibbs(theta = angulosTortugasRad, N = 10)
  #prueba2 <- MHdentroGibbs(theta = angulosTortugasRad, N = 10)
  #prueba <- MHdentroGibbs(theta = angulosTortugasRad, N = 100)
  #prueba <- MHdentroGibbs(theta = angulosTortugasRad, N = 1000)
