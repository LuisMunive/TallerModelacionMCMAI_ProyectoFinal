...
MHdentroGibbs <- function(theta, N, burn.G, ite.inter.G, lag.G){
  ...
  ite=burn.G+ite.inter.G+lag.G*N
  Tburn=burn.G+ite.inter.G
  ...
  #Declarando vectores y contadores a ocupar
  mu1 <- rep(0, N)
  mu2 <- rep(0, N)
  ...
  for(i in 2:ite){
      #set.seed(i)##########################   Fijamos semilla
      muaux1 <- rnorm(1, mean = lambda1_n^(-1) * (lambda1_0 * muaux1 + n * mean(raux * cos(theta))), sd = 1 / sqrt(lambda1_n))
    
      #set.seed(i)##########################   Fijamos semilla
      muaux2 <- rnorm(1, mean = lambda2_n^(-1) * (lambda2_0 * muaux2 + n * mean(raux * sin(theta))), sd = 1 / sqrt(lambda2_n))

      raux <- MetroHast(f = kerY, theta = theta, mu = c(muaux1, muaux2), burn = 1500, ite.inter = 500, lag = 10, tmues = 100, n)
...
tiempoInicio <- Sys.time()
  ...
  prueba3 <- MHdentroGibbs(theta = angulosTortugasRad, N = 1000,burn.G = 500, ite.inter.G =  100, lag = 5)
  #prueba2 <- MHdentroGibbs(theta = angulosTortugasRad, N = 100)
  #prueba2 <- MHdentroGibbs(theta = angulosTortugasRad, N = 1000)

ergomu1<-cumsum(prueba3[[1]])/(1:1000)
plot(ergomu1,type="l",main="Promedios ergodicos de mu1")
ergomu2<-cumsum(prueba3[[2]])/(1:1000)
plot(ergomu2,type="l",main="Promedios ergodicos de mu2")
