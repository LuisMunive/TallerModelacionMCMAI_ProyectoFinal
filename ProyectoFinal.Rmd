...
MHdentroGibbs <- function(theta, N){
  ...
  #library(parallel)
  #n.cores <- round(detectCores(logical = FALSE) / 2)
  #n.cores <- 2
  ...
  for(i in 2:N){
    ...
    for(j in 1:n){
      #Iniciamos con la creacion del cluster
      #cl<-makeCluster(n.cores)
      
      #r[i, j] <-parSapply(cl,X=c(theta[j],theta[j+1]), MetroHast(f = kerY, theta = theta[j], mu = c(mu1[i], mu2[i]), burn = 2500, ite.inter = 500, lag = 1, tmues = 1),simplify = TRUE)
     r[i, j] <- MetroHast(f = kerY, theta = theta[j], mu = c(mu1[i], mu2[i]), burn = 2500, ite.inter = 500, lag = 1, tmues = 1)
